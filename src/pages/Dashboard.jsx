...
const Dashboard = () => {
  ...
  const applyingForPositionId = location.state?.applyingForPositionId;
  ...
  const handleBulkPublish = async () => {
    ...
    try {
      ...
      if (res.ok) {
        alert('🎉 Selected CVs Published Successfully!');
        setSelectedCvIds([]);
      } else {
        let errorMsg = `Failed to publish. Status: ${res.status}`;
        const contentType = res.headers.get('content-type');
        if (contentType && contentType.includes('application/json')) {
          const errorData = await res.json();
          errorMsg = `Error: ${errorData.error || 'Failed to publish'}`;
        }
        alert(errorMsg);
      }
    } catch (error) {
    ...
    <div className="min-h-screen bg-slate-900 text-slate-100 p-8">
      <div className="flex justify-between items-center mb-6 flex-wrap gap-4">
        ...
        <div className="flex gap-3 flex-wrap">
          {applyingForPositionId ? (
            <button onClick={() => {
              const cv = cvs.find(c => c.id === selectedCvIds[0]);
              if (cv) {
                navigate(`/apply/${applyingForPositionId}`, { state: { selectedCvId: cv.id, selectedCvTitle: cv.title } });
              }
            }} disabled={selectedCvIds.length !== 1} className="btn btn-sm btn-success text-white">
              ✅ Add to Application
            </button>
          ) : (
          ...
          <button onClick={() => window.open(`/show-cv/${selectedCvIds[0]}`, '_blank')} disabled={selectedCvIds.length !== 1} className="btn btn-sm btn-accent text-white">📄 Make PDF</button>
          <button onClick={handleBulkPublish} className="btn btn-success btn-sm text-white shadow-md rounded-lg">📢 Publish Selected ({selectedCvIds.length})</button>
        </div>
      </div>

      <div className="overflow-x-auto bg-slate-800 rounded-xl border text-center border-white/10">
        <table className="table w-full text-center text-slate-200">
          <thead>
            <tr className="bg-slate-700/50 text-slate-300 text-center">
          ...
          <tbody>
            {cvs.map(cv => {
              const isLoved = cv.position?.comments?.some(c => c.content === 'REACT:LOVE');
              const isLiked = cv.position?.comments?.some(c => c.content === 'REACT:LIKE');
              return (
                <tr key={cv.id} className="hover:bg-white/5 border-b border-white/5">
                  <td className="text-center">
                    <input type="checkbox" className="checkbox checkbox-sm checkbox-primary" checked={selectedCvIds.includes(cv.id)} onChange={() => handleSelectCv(cv.id)} />
                  </td>
                  <td className="font-semibold text-red-400 cursor-pointer hover:underline" onClick={() => navigate(`/view-cv/${cv.id}`)}>
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div className="form-control w-full sm:col-span-2"> 
                      ...
                      <input type="text" name="positionTitle" placeholder="e.g., Frontend Developer" className="input input-bordered w-full bg-slate-800/50 border-slate-600 rounded-xl text-sm focus:border-purple-500 transition-all" value={formData.positionTitle} onChange={handleChange} />
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-4 text-start">
                    ...
                    <div>
                      <label className="label text-[16px] text-slate-400 font-medium">About Your Projects</label>
                      <textarea name="projects" placeholder="Tell us about your projects..." className="textarea textarea-bordered w-full bg-slate-800/50 border-slate-600 border border-b-gray-600 rounded-xl text-sm h-24 focus:border-purple-500 transition-all" value={formData.projects} onChange={handleChange} />
                    </div>
                  </div>
                </div>
